...
void ofxAssimpModelLoader::updateAnimation(unsigned int animationIndex, float currentTime){
	...
	for (unsigned int i = 0; i < modelMeshes.size(); ++i){
		...
		for( size_t a = 0; a < mesh->mNumBones; ++a)
		{
			...
			if ( a == 7 )
			{
				//bone->mOffsetMatrix.RotationY( 46, boneMatrices[a] );
			}
			...
			while( tempNode)
			{
				...
				if ( a == 7 && tempNode == node && bHeadFollowHack )
				{
					aiMatrix4x4 matY;
					aiMatrix4x4 matZ;
					tempNode->mTransformation.RotationY( atan2( pos.y, ofGetMouseY() + 60.0f ) + ( PI / 4 ) - ( PI / 2 ), matY );
					tempNode->mTransformation.RotationZ( atan2( ofGetMouseX() + 150.0f, pos.x ) + ( PI / 4 ) + ( PI / 2 ) + ( PI / 7 ), matZ );
					boneMatrices[a] = tempNode->mTransformation * matY * matZ * boneMatrices[a];
				}
				else
				{
